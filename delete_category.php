<?php
	...
	$getItem_name = $conn->query("SELECT * FROM `category` WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
	while($g_item = $getItem_name->fetch_array()){
		$curr_item = $g_item['category_name'];
	}

	$conn->query("DELETE FROM `category` WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
