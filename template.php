...
glyphicon glyphicon-pushpin"></i> About</a>
			...
			<div class = "col-lg-9 well" style = "margin-top:60px;">
				<div class = "alert alert-info">Settings / Edit Template</div>
				<?php
					require_once 'connect.php';
					$q_item = $conn->query("SELECT * FROM `template` WHERE `temp_id` = '1'") or die(mysqli_error());
					$f_item = $q_item->fetch_array();
				?>
						<div class = "col-lg-3"></div>
						<div class = "col-lg-6">
							<form method = "POST" action = "edit_temp_query.php" enctype = "multipart/form-data">
								<div class = "form-group">
									<label>Prepared by: </label>
									<input type = "text" required = "required" value="<?php echo $f_item['prepared_by']?>" name = "prepared" class = "form-control" />
								</div>	
								<div class = "form-group">	
									<label>Position</label>
									<input type = "text"  name = "prepared_position" value="<?php echo $f_item['prepared_by_label']?>" required = "required" class = "form-control" />
								</div>	
								<div class = "form-group">	
									<label>Inspected by:</label>
									<input type = "text" name = "inspected" value="<?php echo $f_item['inspected']?>" required = "required" class = "form-control" />
								</div>	
                                <div class = "form-group">	
									<label>Position</label>
									<input type = "text" name = "inspected_position" value="<?php echo $f_item['inspected_by_label']?>" required = "required" class = "form-control" />
								</div>	
								
								<div class = "form-group">	
									<button class = "btn btn-primary" name = "save_temp"><span class = "glyphicon glyphicon-save"></span> Submit</button>
								</div>
							</form>	
					</div>
			</div>
